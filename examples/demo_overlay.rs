...
fn is_mouse_pos_valid(mouse_pos: [f32; 2]) -> bool {
    ...
    ((std::f32::MAX - mouse_pos[0].abs()) > std::f32::EPSILON)
        && ((std::f32::MAX - mouse_pos[1].abs()) > std::f32::EPSILON)
}

fn demo_system(
    _: &mut amethyst::core::legion::world::World,
) -> Box<dyn amethyst::core::legion::schedule::Schedulable> {
    let mut corner: i32 = 0;
    let mut open = true;

    SystemBuilder::<()>::new("DemoSystem")
        .read_resource::<Arc<Mutex<amethyst_imgui::ImguiState>>>()
                ...
                amethyst_imgui::with(|ui| {
                    ...
                    window.build(ui, || {
                        ...
                        ui.menu(&label, true, || unsafe {
                            if imgui::MenuItem::new(&im_str!("Custom"))
                                .selected(corner == -1)
                                .build(ui)
                            {
                                corner = -1;
                            }
                            if imgui::MenuItem::new(&im_str!("Top-Left"))
                                .selected(corner == 0)
                                .build(ui)
                            {
                                corner = 0;
                            }
                            if imgui::MenuItem::new(&ImString::new("Top-Right"))
                                .selected(corner == 1)
                                .build(ui)
                            {
                                corner = 1;
                            }
                            if imgui::MenuItem::new(&ImString::new("Bottom-Left"))
                                .selected(corner == 2)
                                .build(ui)
                            {
                                corner = 2;
                            }
                            if imgui::MenuItem::new(&ImString::new("Bototm-Right"))
                                .selected(corner == 3)
                                .build(ui)
                            {
                                corner = 3;
