//! This is copied from imgui_winit_support but for winit 19.
//!
...
pub fn handle_mouse_scroll_delta(imgui: &mut ImGui, delta: MouseScrollDelta, window_hidpi_factor: f64, app_hidpi_factor: f64) {
	match delta {
		MouseScrollDelta::LineDelta(_, y) => imgui.set_mouse_wheel(y),
		MouseScrollDelta::PixelDelta(pos) => {
			let pos = pos.to_physical(window_hidpi_factor).to_logical(app_hidpi_factor);
...
pub fn handle_event(imgui: &mut ImGui, event: &Event, window_hidpi_factor: f64, app_hidpi_factor: f64) {
...
pub fn handle_window_event(imgui: &mut ImGui, event: &WindowEvent, window_hidpi_factor: f64, app_hidpi_factor: f64) {
	...
	match event {
		...
		CursorMoved { position, modifiers, .. } => {
			let position = position.to_physical(window_hidpi_factor).to_logical(app_hidpi_factor);
