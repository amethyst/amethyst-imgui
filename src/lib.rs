...
use amethyst::{
    core::SystemDesc,
    derive::SystemDesc,
    ecs::{DispatcherBuilder, World, Read, Write, System, ReadExpect, WriteExpect, SystemData},
    error::Error,
    input::{InputEvent, BindingTypes},
    renderer::{
...
impl<'s, T: BindingTypes> System<'s> for ImguiInputSystem<T> {
    ...
    fn run(&mut self, (context, mut platform, input_events, winit_events, mut filtered_events, window): Self::SystemData) {
        ...
        for input in input_events.read(&mut self.input_reader) {
            match input {
                InputEvent::MouseMoved{..} | InputEvent::MouseButtonPressed(_) | InputEvent::MouseButtonReleased(_) | InputEvent::MouseWheelMoved(_) => {
...
impl<'a, 'b, T: BindingTypes> SystemDesc<'a, 'b, ImguiInputSystem<T>> for ImguiInputSystemDesc<T> {
    fn build(self, world: &mut World) -> ImguiInputSystem<T> {
        ...
        ImguiInputSystem { input_reader, winit_reader }
    }
}
...
impl<B: Backend, T: BindingTypes> RenderPlugin<B> for RenderImgui<T> {
    fn on_build<'a, 'b>(
        ...
        dispatcher.add(ImguiInputSystemDesc::<T>::default().build(world), "imgui_input_system", &["input_system", "window"]);
