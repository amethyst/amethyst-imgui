...
impl ImguiState {
	pub fn new(res: &mut amethyst::ecs::Resources, config: ImguiConfig) -> Self {
		...
		imgui.set_ini_filename(config.ini.as_ref().map(imgui::ImString::new));
...
impl<'s> amethyst::ecs::System<'s> for System {
	...
	fn run(&mut self, (events, dimensions, mut imgui_state, mut window): Self::SystemData) {
		let dimensions: &ScreenDimensions = &dimensions;

		if imgui_state.config.screen_dimensions.is_none() || *imgui_state.config.screen_dimensions.as_ref().unwrap() != *dimensions {
		...
		for event in events.read(self.reader.as_mut().unwrap()) {
			winit_support::handle_event(&mut imgui_state.imgui, event, dimensions.hidpi_factor(), 1.0);
		}
