...
impl<'s> amethyst::ecs::System<'s> for BeginFrame {
    ...
    fn run(&mut self, (events, dimensions, time, mut imgui_state, mut window): Self::SystemData) {
        ...
        if imgui_state.config.screen_dimensions.is_none()
            || *imgui_state.config.screen_dimensions.as_ref().unwrap() != *dimensions
        {
            imgui_state.size = (dimensions.width() as u16, dimensions.height() as u16);
            imgui_state
                .imgui
                .set_font_global_scale(dimensions.hidpi_factor() as f32);
            imgui_state.config.screen_dimensions = Some(dimensions.clone());
        }

        for event in events.read(self.reader.as_mut().unwrap()) {
            winit_support::handle_event(
                &mut imgui_state.imgui,
                event,
